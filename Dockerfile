...
CMD echo "Port: ${PORT}" && echo "SBH URL: ${SBH_URL}" && echo "COLLECTION: ${COLLECTION}" && python3 $PYTHONPATH/intent_parser/server/intent_parser_server.py --username sd2e -p $SBH_PASSWORD -s $SBH_URL -l $PORT -c $COLLECTION -i $DICT_ID -a $AUTHN 
